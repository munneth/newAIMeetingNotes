...
def extract_meeting_info_from_url(meeting_url):
    ...
    try:
        ...
        # Extract password from query parameters or path
        password = None
        
        # Check if password is in query parameters
        query_params = parse_qs(parsed_url.query)
        if 'pwd' in query_params:
            password = query_params['pwd'][0]
        
        # If no password in query, check if it's in the path after meeting ID
        if not password:
            # Look for password pattern in the URL
        ...
        print(f"Error extracting meeting info from URL: {e}")
        return None, None

def fetch_meeting_data_from_api():
    """Fetch meeting data from the frontend API"""
    try:
        # API endpoint for meetings (adjust URL as needed)
        api_url = "http://localhost:3000/api/meetings"
        ...
        if response.status_code == 200:
            ...
            # Get the most recent meeting or the one that's about to start
            if meetings:
                ...
                for meeting in meetings:
                    try:
                        ...
                        if start_time > current_time:
                            upcoming_meetings.append((start_time, meeting))
                    except:
                        continue
                ...
                if upcoming_meetings:
                    ...
                    meeting_url = next_meeting.get('link', '')
                    meeting_id, password = extract_meeting_info_from_url(meeting_url)
                    ...
                    if meeting_id:
                        return {
                            ...
                            'duration': next_meeting.get('duration', '30'),
        ...
        print(f"Error fetching meeting data from API: {e}")
        return None
    except Exception as e:
        print(f"Unexpected error fetching meeting data: {e}")
        return None
...
class MeetingBot:
    ...
    def load_meeting_data(self):
        """Load meeting data from the frontend API"""
        print("Loading meeting data from API...")
        self.meeting_data = fetch_meeting_data_from_api()
        
        if self.meeting_data:
            print(f"Meeting data loaded successfully:")
            print(f"  Meeting ID: {self.meeting_data['meeting_id']}")
            print(f"  Password: {self.meeting_data['password']}")
            print(f"  Start Time: {self.meeting_data['start_time']}")
            print(f"  Duration: {self.meeting_data['duration']} minutes")
        else:
            print("Failed to load meeting data from API")
    ...
    def join_meeting(self):
        """Join the meeting using data from API"""
        if not self.meeting_data or not self.meeting_data['meeting_id']:
            print("Error: No meeting data available")
            return
        
        meeting_id = self.meeting_data['meeting_id']
        password = self.meeting_data['password'] or ''
        display_name = "Meeting Bot"

        try:
            meeting_number = int(meeting_id)
        except ValueError:
            print(f"Error: Invalid meeting ID format: {meeting_id}")
            return
        ...
        try:
            ...
            if join_result == zoom.SDKERR_SUCCESS:
                print("Successfully initiated join request")
            else:
                print(f"Failed to join meeting. Error code: {join_result}")
